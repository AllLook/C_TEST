...
int[,] Work(int[,] array, int[,] arr)
{
    ...
    if (rows1 == rows2 && colums1 == colums2) // условие что кол во столбцов первого массива равно кол ву строк второго(чтобы матрицы были схожи)
    {
                    ...
                    for (int column2 = 0; column2 < colums2; column2++) // по столбцам второго массива
                    {
                        for (int row3 = 0; row3 < rows1; rows1++) // цикл для нового массива 
                        {
                            for (int column3 = 0; column3 < colums2; column3++)
                            {
                                ...
                                product[row3, column3] = array[row1, column1] * arr[row2,column2]; // Произведение в текущий индекс
